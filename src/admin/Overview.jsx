    ...
    <div className='grid grid-cols-4 gap-4 w-full h-full lg:grid-rows-3 cursor-default'>
        {stats.map((card,index)=>{
            ...
            self-center justify-center py-6 px-4 gap-3 border border-x-0 border-y-1 border-red-500/20 sm:skew-x-2 md:hover:scale-110 transition-all duration-300 ease-in-out '>
                <span className='font-bold text-white  text-3xl sm:sm:-skew-x-2'>{card.num}</span>
                <p className='text-neutral-500 text-center max-w-[80px] self-center sm:sm:-skew-x-2'>{card.being}</p>
                <span className={`${card.color } sm:sm:-skew-x-2 font-semibold`}>{card.change}</span>
            </div>
            )
        })}
        <div className='col-span-4 lg:row-span-2  bg-neutral-800  border border-x-0 border-white/20 shadow-2xl flex flex-col sm:skew-x-1  p-10 gap-6 h-fit'>
            <span className='text-white text-2xl font-bold sm:sm:-skew-x-1'>Recent Check-ins</span>
            {people.map((person,index)=>{
                return(
                    <div key={index} className='flex justify-between border-b-4 border-neutral-700 py-2 last:border-0 sm:sm:-skew-x-1'>
